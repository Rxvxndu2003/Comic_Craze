@extends('Layout.layoutaccessory')

@section('header')
    <h1 class="display-3 text-white mb-3 animated slideInDown">Accessory Details</h1>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb justify-content-center text-uppercase">
            <li class="breadcrumb-item"><a href="/">Home</a></li>
            <li class="breadcrumb-item"><a href="#">Pages</a></li>
            <li class="breadcrumb-item text-white active" aria-current="page">Accessory Details</li>
        </ol>
    </nav>
@endsection
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="product-image">
                    <img src="/storage/{{ $quick->image }}" alt="{{ $quick->name }}" class="img-fluid rounded shadow">
                </div>
            </div>
            ...
            <div class="col-lg-6">
                <div class="product-details">
                    <h2 class="display-5">{{ $quick->name }}</h2>
                    <p class="fs-4 text-danger mb-3">Rs.{{ number_format($quick->price, 2) }}</p>
                    <div class="ratings mb-3">
                        @for ($i = 0; $i < 5; $i++)
                            <i class="bi {{ $i < $quick->rating ? 'bi-star-fill' : 'bi-star' }} text-warning"></i>
                        @endfor
                        <span class="text-muted ms-2">({{ $quick->rating }} reviews)</span>
                    </div>
                    <p class="mb-4">{{ $quick->description }}</p>
